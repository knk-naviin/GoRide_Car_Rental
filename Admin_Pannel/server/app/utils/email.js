...
const sendBookingEmail = async (bookingDetails) => {
  try {
    ...
    if (
      ...
      !bookingDetails.name ||
      !bookingDetails._id ||
      !bookingDetails.userId?.email // Ensure user email is available
    ) {
      throw new Error("Incomplete booking details or user email not found.");
    }
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: process.env.EMAIL_PASSWORD, // Admin email password or app password
      },
    });
    ...
    const mailOptions = {
      ...
      to: bookingDetails.userId.email, // User's email from `userId`
      subject: "🚗 Your Booking Confirmation - GoRide Car Rental",
      html: `
        <div style="max-width: 600px; margin: auto; font-family: Arial, sans-serif; border: 1px solid #ddd; border-radius: 10px; overflow: hidden; background-color: #f8f9fa; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1);">
          <!-- Header -->
          <div style="background-color: #0a66c2; padding: 20px; text-align: center;">
            <img src="https://res.cloudinary.com/dgroxj0su/image/upload/v1738490354/car-rental/company-logo.png" alt="GoRide Logo" style="max-width: 150px;" />
            <h2 style="color: #fff; margin-top: 10px;">Booking Confirmation</h2>
          </div>

          <!-- Booking Details -->
          <div style="padding: 20px;">
            <p style="font-size: 16px; color: #333;">Hello <strong>${bookingDetails.name}</strong>,</p>
            <p style="font-size: 16px; color: #333;">Your car rental booking has been confirmed. Below are the details:</p>
            <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
              <tr>
                <td style="padding: 10px; background-color: #f1f1f1; font-weight: bold;">Booking ID</td>
                <td style="padding: 10px;">${bookingDetails._id}</td>
              ...
              <tr>
                <td style="padding: 10px; background-color: #f1f1f1; font-weight: bold;">Car Name</td>
                <td style="padding: 10px;">${bookingDetails.carId?.carName || "N/A"}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f1f1f1; font-weight: bold;">From Date</td>
                <td style="padding: 10px;">${new Date(bookingDetails.fromDate).toLocaleDateString()}</td>
    ...
    // Send the email
    await transporter.sendMail(mailOptions);
    console.log(`Email sent successfully to: ${bookingDetails.userId.email}`);
  } catch (error) {
